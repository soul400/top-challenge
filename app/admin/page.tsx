...
export default function AdminDashboard() {
    ...
    try {
      ...
      if (setData.length > 0) { 
        ...
        setSettings({
          ...
          end_datetime: formatForInput(setData[0].end_datetime)
        });
        setSelectedWeek(setData[0].current_week); 
      }
  ...
  const saveSettings = async (e) => {
    ...
    try {
      ...
      alert("✅ تم حفظ التواريخ والأوقات بنجاح");
      ...
      <div className="min-h-screen bg-[#050505] text-white flex items-center justify-center font-sans relative overflow-hidden" dir="rtl">
        ...
        <div className="relative z-10 bg-white/5 backdrop-blur-xl border border-white/10 p-10 rounded-3xl shadow-2xl w-[90%] max-w-md animate-fade-in">
          <div className="text-center mb-8">
            <div className="w-16 h-16 bg-gradient-to-br from-yellow-400 to-yellow-700 rounded-full flex items-center justify-center mx-auto mb-4 shadow-[0_0_20px_rgba(234,179,8,0.4)]"><span className="text-3xl">🦅</span></div>
            <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br from-yellow-300 to-yellow-600">TOP PRO</h1>
            <p className="text-gray-500 text-sm mt-1">بوابة الإدارة المركزية</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-5">
            <div><label className="text-xs text-gray-400 block mb-1">اسم المستخدم</label><input type="text" required value={loginUser} onChange={(e) => setLoginUser(e.target.value)} className="w-full bg-[#111] border border-gray-800 text-white rounded-xl p-3 focus:outline-none focus:border-yellow-500 transition" placeholder="أدخل اسم المستخدم" /></div>
            <div><label className="text-xs text-gray-400 block mb-1">كلمة المرور</label><input type="password" required value={loginPass} onChange={(e) => setLoginPass(e.target.value)} className="w-full bg-[#111] border border-gray-800 text-white rounded-xl p-3 focus:outline-none focus:border-yellow-500 transition" placeholder="••••••••" /></div>
            {loginError && <p className="text-red-500 text-sm font-bold text-center animate-pulse">❌ بيانات الدخول غير صحيحة!</p>}
            <button type="submit" className="w-full bg-gradient-to-r from-yellow-600 to-yellow-500 hover:from-yellow-500 hover:to-yellow-400 text-black font-black text-lg p-3 rounded-xl shadow-[0_0_20px_rgba(234,179,8,0.3)] mt-6 transition-all">تسجيل الدخول 🔒</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-white font-sans overflow-hidden" dir="rtl">
      ...
      <aside className="w-64 bg-[#0a0a0a] border-l border-white/5 flex flex-col relative z-20 shadow-[20px_0_30px_rgba(0,0,0,0.5)]">
        <div className="p-8 pb-4"><h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-br from-yellow-300 to-yellow-600 drop-shadow-lg">TOP PRO</h1><p className="text-gray-500 text-xs font-bold mt-1 tracking-widest uppercase">Admin Terminal</p></div>
        <nav className="flex-1 px-4 py-6 space-y-2">
          {[{ id: 'overview', icon: '📊', label: 'نظرة عامة' }, { id: 'leaderboard', icon: '🏆', label: 'لوحات الصدارة' }, { id: 'players', icon: '👥', label: 'إدارة اللاعبين' }, { id: 'settings', icon: '⚙️', label: 'الإعدادات والجدولة' }].map(item => (
            <button key={item.id} onClick={() => setActiveMenu(item.id)} className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 font-bold ${activeMenu === item.id ? 'bg-gradient-to-l from-yellow-600/20 to-transparent border-r-4 border-yellow-500 text-yellow-500' : 'text-gray-400 hover:bg-white/5'}`}>
              <span className="text-xl">{item.icon}</span> {item.label}
            </button>
          ))}
        </nav>
        <div className="p-6 border-t border-white/5"><button onClick={() => setIsAuthenticated(false)} className="w-full bg-red-900/20 text-red-400 hover:bg-red-900/40 py-2 rounded-lg text-sm font-bold transition">تسجيل الخروج 🚪</button></div>
      </aside>
          ...
          {activeMenu === 'leaderboard' && (
            <div className="animate-fade-in">
              <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div className="flex bg-[#111] p-1 rounded-xl border border-white/5"><button onClick={() => setActiveTab('weekly')} className={`px-6 py-2.5 rounded-lg font-bold text-sm transition ${activeTab === 'weekly' ? 'bg-yellow-600 text-black' : 'text-gray-400 hover:text-white'}`}>الأسبوعية</button><button onClick={() => setActiveTab('global')} className={`px-6 py-2.5 rounded-lg font-bold text-sm transition ${activeTab === 'global' ? 'bg-yellow-600 text-black' : 'text-gray-400 hover:text-white'}`}>العامة (التراكمية)</button></div>
              </div>
                  ...
                  {uniquePlayers.map(user => {
                    ...
                    return (
                      <div key={user.id} className={`flex justify-between items-center p-4 rounded-xl border ${isBanned ? 'bg-red-900/10 border-red-900/50' : 'bg-white/5 border-white/5 hover:border-white/20'}`}>
                        <div><p className={`font-bold text-lg ${isBanned ? 'text-red-400 line-through' : 'text-white'}`}>{user.name}</p><p className="text-gray-500 text-xs font-mono">ID: {user.id} | النقاط الكلية: {user.total_score}</p></div>
